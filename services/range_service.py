...
class RangeService:
    ...
    def find_body_ranges(self,
        ...
        try:
            out = []
            if df is None or df.empty or not {"open", "close"}.issubset(
                    df.columns):
            ...
            for start in range(0, n - lookback + 1, hop):
                ...
                if body_range <= range_size:
                    start_time = df.iloc[start][
                        "time_formatted"] if "time_formatted" in df.columns else str(
                            df.iloc[start]["time"])
                    end_time = df.iloc[end][
                        "time_formatted"] if "time_formatted" in df.columns else str(
                            df.iloc[end]["time"])

                    out.append({
                        "symbol":
                        df.iloc[start]["symbol"]
                        ...
                        round(window_bot, 5),
                        "mid":
                        round((window_top + window_bot) / 2.0, 5),
                        "range_value":
                        round(body_range, 5),
                        "duration_bars":
                        lookback,
    ...
    def fetch_and_calculate_ranges(self,
                                   symbol: str,
                                   timeframe: int = 5,
                                   bars: int = 5520,
                                   lookback: int = 4,
        ...
        try:
            ...
            # Fetch fresh data
            logger.info(
                f"Calculating ranges for {symbol} (tf={timeframe}, bars={bars})"
            )
            ...
            result_data = {
                ...
                "data_start_time":
                str(df.iloc[0]["time"]) if not df.empty else None,
                "data_end_time":
                str(df.iloc[-1]["time"]) if not df.empty else None
            }
            ...
            logger.info(
                f"Calculated {len(body_ranges)} body ranges, {len(merged_ranges)} merged ranges for {symbol}"
            )

            return {"success": True, "cached": False, **result_data}
    ...
    def clear_cache(self, symbol: Optional[str] = None):
        ...
        if symbol:
            keys_to_remove = [
                k for k in self.cache.keys() if k.startswith(symbol)
            ]
